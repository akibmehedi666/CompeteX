...
export default function PeopleDirectory() {
    ...
    const competitions = useMemo(() => {
        ...
        TALENT_POOL.forEach(u => u.competitions?.forEach(c => comps.add(c)));
        return ["All Competitions", ...Array.from(comps)];
    }, []);

    const filteredUsers = TALENT_POOL.filter(user => {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <AnimatePresence mode="popLayout">
                        {filteredUsers.map((user, idx) => (
                            ...
                            >
                                <CommunityCard user={user} onMessage={() => setActiveDirectMessageUser(user)} />
                            </motion.div>
                        ))}
                    </AnimatePresence>
                </div>
...
function CommunityCard({ user, onMessage }: { user: User, onMessage: () => void }) {
    return (
